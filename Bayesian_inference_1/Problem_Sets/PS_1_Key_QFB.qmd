...
Crickets omniscent - know proportion is exactly 30%
...
estL <- exp(mod$coefficients[1])/(1 + exp(mod$coefficients[1]))
...
niter <- 100
CI_dat <- tibble(index = seq(1,niter), mu = numeric(length=niter), lower = numeric(length=niter), upper = numeric(length=niter))
...
CI_dat[ii,] <- tibble(index = ii, mu = muP, lower = muP-SEp, upper = muP+SEp)
...
#consider one case - can you calculate the probability the parameter is true? no - you can just calculate how likely you observe your prop given some parameter estimate.

#point about probability - only comes from a long run with a known true value - for particular dataset, no prob for parameter

#cross
#particluar set, based on allele frequencies at all of the loci contributing, expect 0.18
propLW <- 0.18
Nsamp <- 20
WP <- rbinom(Nsamp,1,propLW)

LWSW <- c(sum(WP),Nsamp - sum(WP))
chiT <- chisq.test(LWSW)
...
Assuming there is a true value - we know how it behaves for the unknown true value. 

You do an experiment once. Don't know that value


  - assumptions, which things are "given"


go through probablity for parameter?
what info would you need to figure this out from a single dataset?
