...
A recent development in evaluation of the relative information in the prior vs. the information gained from the data in the posterior is the `priorsense` package^[Kallioinen, N., T. Paananen, P.-C. Bürkner, and A. Vehtari. 2021. Detecting and diagnosing prior and likelihood sensitivity with power-scaling. http://arxiv.org/abs/2107.14054]. If you are interested is a less technical introduction, here is a short [recent video describing the package](https://www.youtube.com/watch?v=TBXD3HjcIps).
...
- `powerscale_sequence()` evaluates the prior/likelihood sensitivity across a rnag eod powers. This function can be wrapped in either `powerscale_plot_dens()` or `powerscale_plot_quantities()` to plot changes in the posterior densities or the dependency of the posterior on prior or likelihood scaling, respectively.
...
These are three models that you saw in Quantitative Methods 1. We will much of the leave the details of the analysis to you, providing some guidance for three challenging kinds of models to fit and interpret.
