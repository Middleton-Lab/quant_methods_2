...
Our first model will compare the means of two groups. This is like an two-sample *t*-test, also called "independent samples". It is one of the most common statistical tests.
...
We aren't really interested in the posterior means themselves. We are interested in the difference between them (called a "contrast"). This difference tests the hypothesis that the means of the two groups are different from one another (or not).
...
> There is credible difference between the means for the dead vs. alive groups. The peak of the density is about -2, with most of the distribution between -1 and -3.5.
...
> Yes, the interval is from -1.45 to -3.13. So all of the density is far from 0. We don't need to worry about a ROPE or anything here.
