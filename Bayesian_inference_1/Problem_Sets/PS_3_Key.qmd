...
We advise you to restart your R session before you start this section. Load the packages at the top and then begin with loading the earwigs data. That way you will not have any leftover objects in your session that might get called by the new code.
...
The final model is an ANOVA-like model, with one categorical predictor with three levels. The data for this problem come from an experiment designed to test whether appication of light to the eyes would result in a shorter time shift for the test subject.
...
This model is very similar to the first in this problem set. Below we have given you empty headings. Adapt the code from above to test this hypothesis.
...
ggplot() +
  geom_point(data = JL, aes(x = Treatment, y = Shift),
...
pp_dist |> 
  ...
  geom_histogram(bins = 30) +
  scale_fill_manual(values = c("tomato", "steelblue", "darkgreen")) +
