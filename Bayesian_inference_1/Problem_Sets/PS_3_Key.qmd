...
If you find yourself doing this kind of quantitative analysis often, you will also find yourself writing about it often and it's worth spending the time to pick up the relevant LaTeX coding. The basics are pretty straightforward, and you can get very far with just a little bit of knowledge. There are lots of online tutorials and translators.
...
Try out different priors for `a_group` until you find a prior that seems good to you. Because we are not looking at the values for the standard deviation (we just asked you to extract the `b` parameter matrix from the samples, ignoring the standard deviation for now), changing its value won't alter the pattern. If we wanted to, we could choose rows from both priors and simulate data using the full set of parameters, which would be the most correct way to do this simulation. But we should be fine just using values for `horn_length`.
...
First we would like to compare the prior to the posterior.
...
ggplot() +
  geom_density(data = samples, aes(horn_length, color = group, linetype = PP)) +
  scale_color_manual(values = c("tomato", "steelblue")) +
  cowplot::theme_cowplot() +
  labs(x = "Horn Length (mm)", y = "Density")
```

Compare the posterior to the data
...
ggplot() +
  geom_density(data = HL, aes(horn_length, color = group), linetype = "solid") +
  geom_density(data = post_long, aes(horn_length, color = group), linetype = "dotted") +
  scale_color_manual(values = c("tomato", "steelblue")) +
  cowplot::theme_cowplot() +
  labs(x = "Horn Length (mm)", y = "Density")
```
