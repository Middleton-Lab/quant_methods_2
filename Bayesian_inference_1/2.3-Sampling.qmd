...
```{r}
ggplot(Post |> filter(Sample > 2500 & Sample < 2600),
       aes(x = Sample, y = Estimate, color = Parameter)) +
  geom_point() +
  geom_path() +
  scale_color_manual(guide = "none", values = c("#4570E6", "#C32148")) +
...
p1 <- Post |> 
  filter(Parameter == "mu") |> 
  ggplot(aes(Estimate, color = Parameter)) +
  geom_density() +
  facet_grid(Parameter ~ .) +
  scale_color_manual(guide = "none", values = c("#4570E6")) +
...
p2 <- Post |> 
  ...
  geom_density() +
  facet_grid(Parameter ~ .) +
  scale_color_manual(guide = "none", values = c("#C32148")) +
  facet_grid(Parameter ~ ., scales = "free") +
  labs(y = "Density")
...
- Stan uses HMC with a "No U-Turn" sampler [@Carpenter2017-yg; @Hoffman2014-vv]
