...
Plot the distribution of `RPM` ($y$) and a sample of 100 draws from the posterior ($y_{rep}$)
...
10,000 draws for each of 200 combinations of `log10SVL` and `ForagingMode`. Each column is 1 *row* of `pred_values`.
...
- Will be similar for symmetrical distributions
...
- Uses `rope_range()` to automatically find the most suitable range for comparison.
    ...
    - Kruschke also recommends $\pm 0.1 \times sd_y$ 
