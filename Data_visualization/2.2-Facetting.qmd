...
- Dodging and facetting can provide more options
...
- Also set `seed` to make the jittering reproducible
...
- Generate multiple plots, each split by the levels of a factor
- By wrapping with `facet_wrap()`
- By rows or columns with `facet_grid()` if there is some logical order
  - Think about the direction of the comparison and the way your eyes will move around the plot
...
`.` is a placeholder for facetting only by row or column
...
- `free_x`, `free_y`, and `free` can be used to allow `x`, `y`, or both axes to vary.
