...
pwr_biotech <- function(Nbio, varN, varT, Ntech){
  Niter <- 1000
  muN <- 0
  dmus <- 1
  pvals <- rep(NA, Niter)
  ...
  return(mean(pval <= 0.05))
}


#tester <- pmap(var_list[1:3,], pwr_biotech)
...
crossv <- function(jj){
  ...
  fm2 <- lme(WaterTemp ~ AirTempPredicted,
           random = ~ Day | Stream_Name/Site,
           data = stream.train)
  p1 <- predict(fm1, stream.test)
  p2 <- predict(fm2, stream.test)
  return(c(p1, p2))
}
