...
```{r}
set.seed(90843)
Clock <- tibble("ID" = paste0("S",seq(1:100)),"Clock_Allele" = sample(c(rep(1,18),rep(0,82))))
...
bb <- boot(data = Clock$Clock_Allele, statistic = bootfreq, R = 1000,stype = "i")
...
6. Use your preferred iteration method (loop, map, apply, etc.) to perform these steps 10000 times and keep your jackknife and bootstrap CIs in a tibble. You probably want to start with few iterations while you get it working and then scale up. It will take a few minutes to run at 10000. 
