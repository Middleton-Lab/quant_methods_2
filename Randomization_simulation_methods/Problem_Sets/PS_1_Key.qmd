...
Calculate the overall mean and standard deviation for the observed data and use those to generate 10,000 datasets in two groups of the same size as the small data. Then calculate the differences in the means for each iteration and plot them as a histogram. 
...
p2 <- ggplot(data = sims, aes(y)) +
  ...
  scale_x_continuous(limits = c(-12, 12))
plot_grid(p2, p1, nrow = 2, align = "hv")
```
