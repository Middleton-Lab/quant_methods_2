...
MM <- tibble(
  y = c(rnorm(nobs, 17, 2),
        rnorm(nobs, 17, 2)),
  Group = rep(c("Group_1", "Group_2"), each = nobs)
)

lm(y ~ Group, data = MM) |> summary()

write_csv(x = MM, file = "../Data/small_data.csv")
...
write_csv(x = MM, file = "../Data/large_data.csv")
...
> Maybe. There is a lot of variation in group 1, but its mean looks like it might be larger than group 1.
...
> The means differ by 2.4, but that is about as much as the standard deviations.
...
for(ii in 1:niter)
{
 grp1 <- rnorm(nrow(MM[MM$Group=="Group_1",]), grandmu, grandsd)
 grp2 <- rnorm(nrow(MM[MM$Group=="Group_2",]), grandmu, grandsd)
 gdifs[ii] <- mean(grp1) - mean(grp2)
...
> For our seed, the minimum is -4.30 and the maximum is 4.33. The probability of these values is fixed: they are equally likely (though the probability is low). However, taking a larger sample means that we have more chances to randomly select such extreme values.
...
> They are very close with the default quantile (`type = 7`) but not exactly matching. Setting `type = 4` makes them match exactly.
...
> Again they are very close. `qnorm()` gives $\pm$ 1.97, and our values (for this seed) are -1.968 and 1.95.
...
> Normally derived intervals are much too narrow when the data actually follow a *t* distribution (*df* = 8). If there were a lot more data then the intervals would match pretty well.


## Brownian motion and why biological traits are very often normally distributed

Normal or Gaussian distributions (and their relatives like log-Normal) are extremely common distributions in biology, particularly so for phenotypic trait values. Here we want to explore how a simple process can lead to a normal distribution.

The small function below generates a sequence of +1 or -1 for some number of steps (`nsteps`) and then adds the resulting values cumulatively. Think of this process as, at each step, adding or subtracting 1. Because this process doesn't know about its past and only works 1 step at a time, it's called a Markov process.
...
In the chink below, rewrite the `markov_sim()` function to instead use values drawn from a standard normal distribution. This is more typically called Brownian motion, where at each step, a small amount is added (or subtracted).
...
> Because we are adding non-integer values, the resulting distribution is much smoother. However, the range is pretty similar. 


FIXME

This same phenomenon occurs when we repeatedly sample and summarize from a set of observations. Typically when we take a sample from some distribution and compute a summary of some kind, the resulting distribution of values will be normally distributed. Let's use a highly skewed real dataset to see how this works for a simple case, the mean. 

In the Data4Ecologists package, there is a dataset of wolf pack sizes. Load the library and use glimpse to look at the `WolfPackSize` dataset. We'll focus on the `size` column, which is just the number of wolves in each pack that was observed in this study.
...
glimpse(WolfPackSize)

WolfPackSize |>
  ggplot(aes(Size)) +
  geom_histogram(bins = 20, fill = "grey75")


```

> The distribution is right skewed. Most packs are around 5 individuals with a few with many members.
...
mupack <- tibble("mu"= rep(NA,niter))

for(ii in 1:niter)
{
  pack.s <- sample(WolfPackSize$Size, 20)
  mupack[ii,] <- tibble("mu" = mean(pack.s))
  
}

mupack |>
  ggplot(aes(mu)) +
  geom_histogram(bins = 30, fill = "grey75")
...
mupack <- tibble("p5"= rep(NA,niter))

for(ii in 1:niter)
{
  pack.s <- sample(WolfPackSize$Size, 20)
  mupack[ii,] <- tibble("p5" = length(which(pack.s <= 5))/length(pack.s))
  
}

mupack |>
  ggplot(aes(p5)) +
  geom_histogram(bins = 15, fill = "grey75")
