...
1. Sample from distributions and compare to observed values
2. Generate "populations" of parameters and let them "evolve" across geenerations
...
- Choose an error term (MSE, MAE, etc.) or use the likelihood
- Draw data from distributions for each parameter
- Evaluate the parameters given the model
    - Rejection sampling
    - Collect all samples and evaluate later
...
GGsim <- tibble(t = runif(20, min = 0, max = 25),
             y = Gompertz(t, a = 5, b = 1, c = 0.5) + rnorm(20, 0, 0.25))
