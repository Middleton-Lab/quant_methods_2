...
```{r}
mu_ps <- mean(ps$ps)
ci_ps <- c(mu_ps + qt(0.975, length(LineIDs)-1)*sd(ps$ps)/sqrt(length(LineIDs)),
           mu_ps - qt(0.975, length(LineIDs)-1)*sd(ps$ps)/sqrt(length(LineIDs)))
...
ps |>
  ...
  annotate("text", x = 2, y = 120, label = paste0("Estimate: ",round(mu_ps,3)), size = 10) +
  annotate("text", x = 2, y = 100, label = paste0("CI: ",round(ci_ps[2],3)," - ",round(ci_ps[1],3)), size = 10) 
